...
class FaceView: UIView {
    ...
    private func pathForEye(_ eye: Eye) -> UIBezierPath {
        ...
        if eyesOpen {
            path = UIBezierPath(arcCenter: eyeCenter, radius: eyeRadius, startAngle: 0, endAngle: .pi * 2, clockwise: true)
        } else {
            path = UIBezierPath()
    ...
    private func pathForSkull() -> UIBezierPath {
        let path = UIBezierPath(arcCenter: skullCenter, radius: skullRadius, startAngle: 0, endAngle: 2 * .pi, clockwise: false)
        path.lineWidth = lineWidth
        return path
    }
